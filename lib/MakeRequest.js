...
function getRequestOpts(self, requestArgs, spec, overrideData) {
  ...
  const dataFromArgs = util.getDataFromArgs(args);
  ...
  if (args.length) {
    throw new Error(
      `Campzimmer: Unknown arguments (${args}). Did you mean to pass an options object? See https://github.com/stripe/stripe-node/wiki/Passing-Options. (on API request to ${requestMethod} \`${path}\`)`
    );
  }
