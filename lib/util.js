...
const utils = (module.exports = {
  isAuthKey: (key) => {
    ///^(?:[a-z]{2}_)?[A-z0-9]{32}$/.test(key)
    return typeof key == 'string';
  },
  isOptionsHash: (o) => {
    return isPlainObject(o) && OPTIONS_KEYS.some((key) => hasOwn.call(o, key));
  },
  isValidatedQueryObject: (o, keys) => {
    return isPlainObject(o) && Object.keys(o).every((k) => keys.indexOf(k) !== -1);
  },
  capToLowerName: (name) => {
...
function emitWarning(warning) {
  ...
  return process.emitWarning(warning, 'Campzimmer');
}
